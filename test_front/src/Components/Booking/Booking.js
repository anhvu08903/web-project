...
const Booking = () => {
  ...
  const [priceRange, setPriceRange] = useState([0, 100000000]); // State lưu trữ khoảng giá vé
  const [timeRange, setTimeRange] = useState([0, 24]); // State lưu trữ khoảng thời gian đi

  const [timeVal, setValue1] = React.useState([0, 24]);
  const [priceVal, setValue2] = React.useState([0, 100000]);
  ...
  const handleChange2 = (event, newValue) => {
    console.log("day la gia tri moi",newValue)
    setValue2(newValue);
  ...
  const [bookings, setArray] = useState([]); // Mảng các chuyến xe sau khi get từ backend
  ...
  const filterBookings = (timeRange, priceRange) => {
    // console.log("bokk:", bookings, timeRange, priceRange[0]);
