...
const AdminRegister = () => {
    ...
    const validateForm = () => {
        ...
        if (!adminCredentials.adminemail.trim() || !regex.test(adminCredentials.adminemail)) {
            errors.adminemail = "Valid email is required";
        }

        if (!adminCredentials.adminpassword.trim() || adminCredentials.adminpassword.length < 6) {
            errors.adminpassword = "Password must be at least 6 characters long";
        }

        if (adminCredentials.adminpassword !== adminCredentials.adminconfirmPassword) {
    ...
    const handleSubmit = (event) => {
        ...
        if (Object.keys(errors).length === 0) {
            axios
                .post("http://localhost:8080/identity/api/admin/signup", adminCredentials)
                .then((response) => {
